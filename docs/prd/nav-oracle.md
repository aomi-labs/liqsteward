...
Two platform findings from that run, recorded for the platform team: the SDK 5.0.0 digested guard tables through `serde_json` map order, so a plugin had to enable `serde_json/preserve_order` to match the host (fixed in aomi-sdk 5.0.1 by hashing a sorted-key canonical form; the workaround is gone from `aomi-app/Cargo.toml`); and the CLI's provider bootstrap fails silently on the repo's full `providers.toml`, so local runs use a minimal single-fork file.
